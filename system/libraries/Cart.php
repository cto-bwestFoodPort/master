...
class CI_Cart {
	...
	function _insert($items = array())
	{
		...
		$items['price'] = $this->format_number($items['price']);
		// Trim any leading zeros
		...
		{
			//Added 4/10/2015 as a custom fix for foodport.com
			$uniqueableItems = $items;

			foreach($uniqueableItems['options'] as $key=>$value){
				if(is_array($value)){
					$uniqueableItems['options'][$key] = serialize($value);
				}
			}
			///////////////////////////////////////////////////

			$rowid = md5($items['id'].implode('', $uniqueableItems['options']));
		}
		else
		{
